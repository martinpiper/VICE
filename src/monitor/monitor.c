...
void monitor_cpuhistory_store(WORD addr, BYTE op, BYTE p1, BYTE p2,BYTE reg_a,BYTE reg_x,BYTE reg_y,BYTE reg_sp,BYTE reg_st)
{
    ++cpuhistory_i;
...
//        mon_out(".%s:%04x   %s   AC$%0.2x XR$%0.2x YR$%0.2x SP$%0.2x %c%c-%c%c%c%c%c\n", mon_memspace_string[mem], loc, dis_inst, cpuhistory[pos].reg_a, cpuhistory[pos].reg_x, cpuhistory[pos].reg_y, cpuhistory[pos].reg_sp,
        mon_out("%04x  %s%s :A$%0.2x X$%0.2x Y$%0.2x SP$%0.2x %c%c-%c%c%c%c%c\n", loc, dis_inst, padding, cpuhistory[pos].reg_a, cpuhistory[pos].reg_x, cpuhistory[pos].reg_y, cpuhistory[pos].reg_sp,
...
void mon_add_name_to_symbol_table(MON_ADDR addr, char *name)
{
    ...
    if (old_addr >= 0 && old_addr != loc) {
        mon_out("Not changing address of label %s from $%04x to $%04x\n",
                  name, old_addr, loc);
		return;	// MPi: TODO: The next line produces a weird bug with labels not being shown with shl or able to be matched. So for now the below line is not executed.
